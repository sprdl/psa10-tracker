...
Calls
  ...
  made, against its price at the time:
    Buy / Definitely buy: wrong as soon as the ask drops more than THRESHOLD below
        the call price (you could have bought meaningfully cheaper). Right if that
        never happens in the window.
    Watch / Don't buy: right as soon as the ask drops more than THRESHOLD below
        the call price (waiting paid off). Wrong if the window ends with the
        price more than THRESHOLD above it and no such dip. Otherwise neutral.
    Defer: not scored.
...
def build(root: Path = ROOT) -> Path:
    ...
    for lst in runs.values():
        for r in lst:
            ...
            floor = r["price"] * (1 - THRESHOLD)
            dipped = next(((d, p) for d, p in after if p <= floor), None)
            matured = now >= end
            status, why = "pending", ""
            if r["tag"] in BUYISH:
                if dipped:
                    status, why = "wrong", f"dropped to ¥{dipped[1]:,} on {dipped[0][:10]}"
                elif matured:
                    status, why = "right", f"never more than {int(THRESHOLD*100)}% cheaper in {WINDOW_DAYS} days"
            elif r["tag"] in WAITISH:
                if dipped:
                    status, why = "right", f"dropped to ¥{dipped[1]:,} on {dipped[0][:10]}"
                elif matured:
                    if last and last[1] >= r["price"] * (1 + THRESHOLD):
    ...
    out.write_text(json.dumps({"as_of": as_of, "window_days": WINDOW_DAYS, "threshold": THRESHOLD,
